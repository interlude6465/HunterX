...
class CombatAI {
  ...
  abortCombat(reason = 'aborted') {
    ...
    if (this.mainCombatLoop) {
      clearInterval(this.mainCombatLoop);
      this.mainCombatLoop = null;
    }
  ...
  async engageTarget(entity) {
    ...
    try {
      ...
      // Equip combat gear
      if (this.bot.equipmentManager) {
        await this.bot.equipmentManager.switchToCombatMode();
      }
      
      // Attack the target
      await this.handleCombat(entity);
  ...
  startMainCombatLoop() {
    ...
    this.mainCombatLoop = safeSetInterval(async () => {
      // If no current target but still have attackers
      if (!this.currentTarget && this.activeAttackers.size > 0) {
        console.log(`[COMBAT] Resuming combat, finding new target`);
        this.switchToOptimalTarget();
      }
      
      // If we have a current target
      if (this.currentTarget && this.bot.entities[this.currentTarget.id]) {
        const distance = this.bot.entity.position.distanceTo(this.currentTarget.position);
        
        // Make sure we're attacking
        if (!this.bot.entity.isCollidingWith(this.currentTarget)) {
          ...
          if (distance > 4) {
            try {
              ...
              // Pathfinding failed, continue anyway
            }
          }
        }
        ...
        if (this.bot.pvp) {
          try {
            ...
            // Attack failed, target might be dead
          }
        }
        
        // Log status periodically
        if (config.combat.multiTarget.logCombatStatus && Math.random() < 0.1) { // 10% chance each cycle
          console.log(`[COMBAT] Attacking: ${this.currentTarget.name || this.currentTarget.username || 'Unknown'} (${distance.toFixed(1)}m)`);
        }
    ...
    console.log('[COMBAT] Main combat loop started');
  }
  ...
  bot.once('spawn', async () => {
    ...
    bot.on('entityHurt', async (entity) => {
      if (entity === bot.entity) {
        ...
        if (attacker) {
          ...
          // Add to multi-target combat system if enabled
          if (config.combat.multiTarget.enabled && combatAI && combatAI.activeAttackers) {
            combatAI.activeAttackers.add(attacker.id);
            console.log(`[COMBAT] Added attacker to multi-target list: ${attackerName}`);
          }
        ...
        // Handle combat with multi-target system or legacy system
        if (attacker && combatAI) {
          if (config.combat.multiTarget.enabled) {
            // Multi-target combat system will handle this automatically
            console.log(`[COMBAT] Multi-target system handling attack from ${attacker.username || attacker.name || 'Unknown'}`);
          } else if (!combatAI.inCombat && config.combat.autoEngagement?.autoRetaliate) {
            // Legacy single-target combat system
    ...
    bot.on('entityDead', (entity) => {
      if (entity.type === 'player' && combatAI && combatAI.currentTarget === entity) {
        ...
        // Remove from multi-target attackers list
        if (config.combat.multiTarget.enabled && combatAI.activeAttackers) {
          combatAI.activeAttackers.delete(entity.id);
        }
      }
      
      // Also handle mob deaths
      if (entity !== bot.entity && combatAI && combatAI.activeAttackers && combatAI.activeAttackers.has(entity.id)) {
        console.log(`[COMBAT] Killed ${entity.name || 'mob'}!`);
        combatAI.activeAttackers.delete(entity.id);
      }
    });
    
    // Periodic combat status logging
    if (config.combat.multiTarget.enabled && config.combat.multiTarget.logCombatStatus) {
      safeSetInterval(() => {
        if (combatAI && combatAI.activeAttackers && combatAI.activeAttackers.size > 0) {
          combatAI.logCombatStatus();
        }
      }, 5000, 'CombatStatusLogger');
    }
